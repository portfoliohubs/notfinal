...
export default function Header({ showBack }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-border/80 bg-background/90 backdrop-blur-xl supports-[backdrop-filter]:bg-background/60">
      <div className="max-w-7xl mx-auto flex h-16 items-center justify-between px-4 sm:px-6">
            ...
            <div className="flex items-center gap-2.5 cursor-pointer group">
              {(CONFIG.brand.logoUrl || CONFIG.brand.favicon) ? (
                <img 
                  ...
                  onError={(e) => {
                    // Try remote if local fails, otherwise fallback icon
                    if (e.currentTarget.src.endsWith('/logo.png')) {
                      e.currentTarget.src = CONFIG.brand.logoUrl || CONFIG.brand.favicon;
                    } else {
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <nav className="hidden items-center gap-4 text-xs font-semibold text-muted-foreground lg:flex" aria-label="Primary">
            <Link href="/website" className="transition-colors hover:text-foreground">Website</Link>
            <Link href="/cv" className="transition-colors hover:text-foreground">CV</Link>
